...
{
    public partial class DeleteSaleLog : Form
    {

        #region Essential data
        readonly SalesLog callerForm;

        public DeleteSaleLog(SalesLog r)
        {
            InitializeComponent();
            callerForm = r;
        ...
        {
            using (SqlCommand checkCmd = new SqlCommand("GetExistedSalesLogNumberById", Shared.conn))
            {
            ...
            if (User_Entered_Sale_Id())
                if (DoesSaleAlreadyExist())
                {
                    ...
                    if (delete == DialogResult.Yes)
                    {
                        try
                        {
                            using (SqlCommand cmd = new SqlCommand("DeleteSaleLog", Shared.conn))
                            {
                                ...
                                if (rowsAffected > 0)
                                    Shared.ProcessIsDoneMessageBox("sale", "canceled");
                                else
