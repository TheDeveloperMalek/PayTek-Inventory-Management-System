...
namespace Inventory_Manager
{
    public partial class EditSale : Form
    {
        ...
        private bool CheckIfThereIsEnoughProductQuantityUpdate(int wanted_quantity)
        {
            ...
            {
            cmd.CommandText = $@"SELECT ""Product Name"" from Sale WHERE id = {SaleIdTextBox.Text}";
            var name = Convert.ToString(cmd.ExecuteScalar());
            cmd.CommandText = $@"SELECT quantity FROM Sale WHERE id = {SaleIdTextBox.Text}";
            var old_quantity_of_Sale = Convert.ToInt32(cmd.ExecuteScalar());
            cmd.CommandText = $@"SELECT p.quantity from Product p JOIN Sale pu ON pu.""Product ID"" = p.id WHERE pu.id = {SaleIdTextBox.Text}";
                ...
                if (User_Entered_Quantity())
                    if (DoesSaleAlreadyExist())
                    {
                        if (CheckIfThereIsEnoughProductQuantityUpdate(int.Parse(QuantityTextBox.Text)))
                            try
                            {
                                using (SqlCommand cmd = new SqlCommand("EditSaleQuantity" , Shared.conn))
                                {
