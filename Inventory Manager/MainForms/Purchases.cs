    ...
    public partial class Purchases : Form
    {
        ...
        private bool Check_If_There_Is_Enough_Product_Quantity(int wanted_quantity)
        {
            ...
            if (product_name_text_box.Text != "")
            {
                ...
                Shared.ErrorOccuredMessageBox($"Unavailabe because the available quantity of product \"{product_name_text_box.Text}\" is {quantity}");
                return false;
            }
                    ...
                    if (Check_If_Purchase_Already_Exists())
                    {
                        try
                        {
                            using (SqlCommand cmd = Shared.conn.CreateCommand())
                            {
                                ...
                                if (rowsAffected > 0)
                                {
                                    Shared.ProcessIsDoneMessageBox("purchase" , "edited");
                                }
                                else
                                {
                    ...
                    if (Check_If_Purchase_Already_Exists())
                    {
                        try
                        {
                            using (SqlCommand cmd = Shared.conn.CreateCommand())
                            {
                                ...
                                if (rowsAffected > 0)
                                {
                                    Shared.ProcessIsDoneMessageBox("purchase" , "deleted");
                                }
                                else
                                {
                                    Shared.ErrorOccuredMessageBox("No record found with the specified id");
